...
english_alphabet = [(('A', 'E', 'I', 'O', 'U', 'L', 'N', 'S', 'T', 'R'), 1),
                    (('D', 'G'), 2),
                    (('B', 'C', 'M', 'P'), 3),
                    (('F', 'H', 'V', 'W', 'Y'), 4),
                    (('K'), 5),
                    (('J', 'X'), 8),
                    (('Q', 'Z'), 10)] 
russian_alphabet = [(('А', 'В', 'Е', 'И', 'Н', 'О', 'Р', 'С', 'Т'), 1),
                    (('Д', 'К', 'Л', 'М', 'П', 'У'), 2),
                    (('Б', 'Г', 'Ё', 'Ь', 'Я'), 3),
                    (('Й', 'Ы'), 4),
                    (('Ж', 'З', 'Х', 'Ц', 'Ч'), 5),
...
alphabet = int(input('Выберете алфавит, 0 - английский, 1 - русский: '))

if alphabet == 0:
    word = input('Введите слово: ')
    sum_point = 0
    for letter in word:
        for let, point in english_alphabet:
            for i in let:
                if letter.upper() == i:
                    sum_point += point
else:
    ...
    sum_point = 0
    for letter in word:
        for let, point in russian_alphabet:
            for i in let:
                if letter.upper() == i:
                    sum_point += point

print(f'За слово "{word}" вы получаете {sum_point} очк.')
